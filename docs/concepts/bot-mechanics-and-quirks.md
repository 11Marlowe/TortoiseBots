...
| :--- | :--- | :--- |
| **Tank** | `TankTargetValue` (`FindTankTargetSmartStrategy`) | **Bucketed Aggro Priority:** attackers are bucketed — loose mobs the tank holds nothing on first (nearest), then held mobs in melee reach, then held mobs out of reach; lowest personal threat breaks ties among held mobs. `TankAssistTrigger` only switches while the tank still holds its current target (`has aggro`), so the tank finishes its mob and can switch back later. Priority 1 = Explicit `.bot action attack` target; Priority 2 = Raid Target Icon; strictly ignores CC targets. |
