...
| **Rebuild the bot pool after changing bot gear, skills or professions** | Set `AiPlayerbot.RandomBotPoolReset = once:<new-token>` (and `RandomBotAutoCreate = 1`), restart, then watch `bot pool status` → [Resetting the managed bot pool](guides/living-world.md#resetting-the-managed-bot-pool) |
